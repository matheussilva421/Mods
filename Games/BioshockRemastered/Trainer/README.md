...
- Build Epic confirmada por `Version.ini`: `Configuration=bioshockpcpatchfinal` / `ChangeNumber=127355` (suporte parcial)
...
Na build Epic `ChangeNumber=127355`, `Lock Consumables` fica bloqueado por seguranca. Os demais toggles usam AOBs encontrados/portados do `BioshockHD.exe` Epic fornecido.
...
O executavel compila e empacota os hooks portados da tabela CE para Steam/GOG `v1.0.122872`. A build Epic confirmada pelo `Version.ini` enviado usa `ChangeNumber=127355`; o `BioshockHD.exe` Epic fornecido confirmou AOBs suficientes para liberar God Mode, Invisible, 1-Hit Kill Enemy, No Alerts, Protect Little Sister e Unlock Gene Slots. `Lock Consumables` continua bloqueado na Epic porque varios caminhos de consumo foram recompilados. Como a propria tabela avisa, fast travel, cutscenes, retorno ao menu e troca entre DLC/main game podem recriar ponteiros; use `F12` antes dessas transicoes.

Para completar a Epic, o proximo passo e portar especificamente os caminhos de `Lock Consumables` que nao batem com a CT Steam/GOG.
