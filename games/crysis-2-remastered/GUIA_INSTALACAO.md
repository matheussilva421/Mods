# Guia de Instalacao e Uso — Crysis 2 Remastered (Steam Deck / SteamOS)

---

## Metodo Simples (Recomendado) — autoexec.cfg

O Crysis 2 Remastered usa CryEngine 3, que carrega automaticamente um arquivo chamado
...
3. Execute o script de instalacao:
   ...
   ```bash
   bash ~/tools/crysis-2-remastered-trainer/steamdeck/install_steamdeck.sh
   ```
...
1. Localize a pasta raiz do jogo no Steam Deck:
   - `~/.steam/steam/steamapps/common/Crysis 2 Remastered/`
   - ou `~/.local/share/Steam/steamapps/common/Crysis 2 Remastered/`
   - Se o jogo esta num cartao SD: `/run/media/mmcblk0p1/steamapps/common/Crysis 2 Remastered/`

2. Copie o arquivo `autoexec.cfg` para dentro dessa pasta:

   ```bash
   cp autoexec.cfg ~/.steam/steam/steamapps/common/"Crysis 2 Remastered"/
   ```
...
2. Clique com botao direito em **Crysis 2 Remastered**
...
- O Crysis 2 usa o sistema Nanosuit 2.0 com armadura integrada
...
- Copie manualmente para o caminho do SD:
  ```bash
  cp autoexec.cfg /run/media/mmcblk0p1/steamapps/common/"Crysis 2 Remastered"/
  ```
...
```
games/crysis-2-remastered/
├── autoexec.cfg                     <- arquivo drop-in (este e o principal)
├── crysis2_remastered_basic.ct      <- tabela Cheat Engine (metodo avancado)
...
- Crysis 2 Remastered instalado via Steam
...
4. Reinicie a Steam
5. No Crysis 2 Remastered, habilite compatibilidade e selecione SteamTinkerLaunch
...
9. Configure o comando custom do STL para:
   - `/home/deck/tools/crysis-2-remastered-trainer/steamdeck/stl_launch.sh %command%`
10. Se necessario, configure variaveis de ambiente:
    - `CE_BIN=/home/deck/tools/cheatengine/cheatengine-x86_64`
    - `TABLE_PATH=/home/deck/tools/crysis-2-remastered-trainer/crysis2_remastered_basic.ct`

**ATENCAO:** A tabela `crysis2_remastered_basic.ct` foi entregue como template.
