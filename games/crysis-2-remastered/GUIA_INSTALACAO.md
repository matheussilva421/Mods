# Guia de Instalacao — Crysis 2 Remastered (Steam Deck / PC)
...
1. Steam > botao direito em **Crysis 2 Remastered** > Propriedades
...
| `i_unlimitedammo 1` | Municao infinita | Nao |
...
├── autoexec.cfg                     <- principal (drop-in)
├── crysis2_remastered_basic.ct      <- tabela Cheat Engine (avancado)
├── GUIA_INSTALACAO.md               <- este arquivo
└── steamdeck/
    ├── install_steamdeck.sh         <- instalacao automatica
    ├── stl_launch.sh                <- launcher CE via STL
    ├── check_build.sh               <- verifica versao da build
    ├── build.lock.example           <- modelo de lock de versao
    └── stl_profile.env              <- variaveis de ambiente STL
```
...
3. Reinicie o Steam
4. No Crysis 2, habilite compatibilidade e selecione SteamTinkerLaunch
...
7. No STL, configure o comando customizado:
   ```
   ~/tools/crysis-2-remastered-trainer/steamdeck/stl_launch.sh %command%
   ```
8. Configure as variaveis:
   ```
   CE_BIN=~/tools/cheatengine/cheatengine-x86_64
   TABLE_PATH=~/tools/crysis-2-remastered-trainer/crysis2_remastered_basic.ct
   ```
